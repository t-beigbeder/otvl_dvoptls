terraform {
  required_providers {
    openstack = {
      source = "terraform-provider-openstack/openstack"
    }
  }
}

resource "openstack_compute_keypair_v2" "this" {
  name       = var.ssh_key_name
  public_key = var.ssh_pub
}

resource "openstack_networking_port_v2" "ext" {
  count = length(var.instances_attrs)
  network_id = var.ext_net_id
  security_group_ids = [
    var.external_sg_id
  ]
  admin_state_up = "true"
}

resource "openstack_compute_instance_v2" "instance" {
  name        = var.instance_attrs.name
  image_name  = var.instance_attrs.image_name
  flavor_name = var.instance_attrs.flavor_name
  key_pair    = openstack_compute_keypair_v2.this.name
  user_data   = var.instance_attrs.user_data
  security_groups = []
  network {
    port = openstack_networking_port_v2.ext[count.index].id
  }
  metadata = {
    "groups"    = var.instance_attrs.groups
    "otvl_meta" = var.instance_attrs.otvl_meta
  }
  lifecycle {
    ignore_changes = [security_groups]
  }
}
